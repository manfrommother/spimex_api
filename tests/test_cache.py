...
class TestRedisCache:
    ...
    def test_get_ttl_before_target(self, mock_datetime, redis_mock):
        """
        Тест вычисления TTL когда текущее время меньше целевого времени сброса кеша
        """
    ...
    def test_get_ttl_after_target(self, mock_datetime, redis_mock):
        """
        Тест вычисления TTL когда текущее время больше целевого времени сброса кеша
        """
        # Настраиваем текущее время на 15:00
        ...
        # Ожидаемое время до сброса: от 15:00 до 14:11 следующего дня = 23 часа 11 минут = 83460 секунд
        expected_ttl = (23 * 60 * 60) + (11 * 60)
        assert ttl == expected_ttl
    
    def test_get_existing_data(self, redis_mock):
        """Тест получения существующих данных из кеша"""
        # Настраиваем мок для redis_client.get
        ...
        # Проверяем, что результат был правильно десериализован
    ...
    def test_set_with_default_ttl(self, mock_get_ttl, redis_mock):
        ...
        mock_get_ttl.return_value = 3600
